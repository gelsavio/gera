...
test('HTML, manifesto e SERVICE WORKER carregam uma vez o módulo da 8H',function(){
 ...
 assert.ok(sw.includes("const CACHE_NAME = CACHE_PREFIX + 'v3.15.29';"));assert.equal(JSON.parse(fs.readFileSync(path.join(root,'manifest.json'),'utf8')).version,'3.15.29');
...
test('recursos funcionais fora do escopo permanecem byte a byte iguais à 3.15.27',function(){
 const files=['offline.html','manual-gera.html','styles/inline-style-01.css','js/storage.js','js/chords.js','js/state.js','js/audio/core.js'];
