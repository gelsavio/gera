...
test('HTML e SERVICE WORKER carregam uma vez o único módulo novo da 8G',function(){
 ...
 const added=fs.readdirSync(path.join(root,'js','ui')).filter(function(file){return file!=='settings-modals.js'&&file!=='lyrics-editor.js'&&!fs.existsSync(path.join(previous,'js','ui',file))});
